...
Т.е просверливать props - мы сквозь компоненты где они не нужны. Это не удобно
...
Action - это объект. Объект содержит информацию о типе события и какую-нибудь сопроводительную информацию
    ...
    На основании информации которую содержит объект action, наш reducer - это некоторая функция должен преобразовать в наш store
...
action creators - функции которые создают actions. И возвращает объект с динамическими значениями
...
И если никаких видоизменений в рамках reducers не должно происходить, то он просто должен
...
Через useSelector благодаря Provider мы получаем данные из нашего store
...
onClick={() => dispatch(addTodo(todo.id))} // todo.title?
...
Он будет содержать корневой reducer который будет объеденять в combaineReducer все остальные reducers
...
Т.к в процессе работы может произойти опечатка и мы не сможем ее отследить
type: ADD_TODO, 
...
next(action) - гарантирует нам что после 21 строчки у нас отработает reducer. Измениться состояние
    Случиться это синхронно. И только после того как у нас reducer отработает мы попадем на строчку 23
...
Если мы используем библиотеку как внешний инструмент мы должны использовать hepler persistedReducer
    передать туда настройки нашего persist и передать туда наш reducer (rootReducer)
...
При этом в рамках middleware может случиться не один action 
    ...
    - Ошибка
        ...
        Чтобы у нас на уровне приложения была информация об ошибках и могли пользователю отрисовать ошибку
...
Описание работы клиентского api рассмотрен в файле ./reduxThunk/api/index.js
